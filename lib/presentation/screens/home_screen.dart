...
class HomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Home is the root of the app's `.go()` hub (see `SystemBackHandler`'s
    // own header) — there is nowhere left to navigate back to, so system
    // back here means the player is trying to leave the app entirely.
    // Rather than let that close it silently, ask first.
    void confirmExit() {
      unawaited(ExitConfirmationDialog.show(context, ref));
    }
    ...
    return SystemBackHandler(
      ...
      child: AppBackground(
        child: Scaffold(
          ...
          // `game_screen.dart`'s matching note for why this is a token at
          // zero alpha rather than `Colors.transparent`.
          backgroundColor: tokens.colors.background.withValues(alpha: 0),
          appBar: AppBar(
            title: Text(l10n.navHome),
            actions: [
              // The only entry point into `SettingsRoute` anywhere in the
              // live app — every other reference to it was either a test
              // driving the router directly or `StubScreen`'s dev-only route
              // switcher, which no route has built since P11/P17 gave every
              // screen a real builder. Without this icon the screen existed
              // but no player could ever reach it.
              IconButton(
              ...
              // Ch10's ONE permitted network surface: a small static icon.
              // Never a dialog, never a banner, never a retry button — see
          ...
          body: SafeArea(
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  const _StreakBanner(),
                  // Invisible — see its own doc for why the streak banner is
                  // the trigger rather than first launch or app resume.
                  const _NotificationPermissionRequester(),
                  const SizedBox(height: AppTokens.space16),
                  const CoinBalanceTile(),
                  // Renders nothing (and no extra gap) until level 8 is
                  // completed and stays dismissible after that — see its own
                  // doc.
                  ...
                  const SizedBox(height: AppTokens.space12),
                  // Same bug class as the Settings gear icon above:
                  // `LeaderboardRoute` rendered fine and every route-level
                  // test could reach it by driving the router directly, but
                  // nothing in the live app ever navigated there.
                  OutlinedButton(
                    onPressed: () {
                      ref.tapFeedback();
